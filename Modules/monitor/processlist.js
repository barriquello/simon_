...
var processlist_ui =
{
    ...
    'init':function()
    {
        ...
        if (this.monitors[this.monitorid].decoder.variables[this.variableid].processlist==undefined) this.monitors[this.monitorid].decoder.variables[this.variableid].processlist = "";
        this.variableprocesslist = this.decode(this.monitors[this.monitorid].decoder.variables[this.variableid].processlist);
        ...
        for (i in processlist_ui.inputlist) {
          ...
          out += "<option value="+input.id+">Monitor "+input.monitorid+":"+input.name+" "+input.description+"</option>";
        }
        $("#input-select").html(out);

        // Feedlist
        var out = "<option value=-1>CRIAR NOVO:</option>";
    ...
    'draw':function()
    {
        ...
        if (this.variableprocesslist.length==0) {
            out += "<tr class='alert'><td></td><td></td><td><b>Nenhum processo definido</b></td><td></td><td></td><td></td></tr>";
        } else {
            ...
            for (z in this.variableprocesslist)
            {
                ...
                if (this.processlist[processid][1]==1) {
                    ...
                    arg += "Monitor "+this.inputlist[inpid].monitorid+": ";
